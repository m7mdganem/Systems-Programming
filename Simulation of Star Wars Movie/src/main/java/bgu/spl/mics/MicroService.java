...
public abstract class MicroService implements Runnable {
    ...
    /*
     * the {@code callbackMap} field maps each event to the appropriate callback function
     * to be called when messages are taken from the micro-service messages-queue.
     */
    protected ConcurrentHashMap<Class<? extends Message> , Callback> callbackMap = new ConcurrentHashMap<>();
    /*
     * the {@code futures} field is a hash-map which maps each event to its future
     * (the future returned from the sendEvent method).
     */
    protected ConcurrentHashMap<Event<?> , Future<?>> futures = new ConcurrentHashMap<>();
    /*
     * the {@code terminate} is a boolean indicated if the fetching messages loop in the rum method should stop
     * (if the microservice should terminate).
     */
    private boolean terminate = false;
    //The {@code messageBus} field is an instance of the MessageBusImpl singleton
    private final MessageBus messageBus = MessageBusImpl.getInstance();
    private final String name; //The name of the microservice
    //The {@code diary} field is an instance of the Diary singleton
    protected final Diary diary = Diary.getInstance();
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        /*
         * first store the callback supplied to the callbackMap, where when trying to process
         * the message later we can call the appropriate callback function.
         * we use putIfAbsent because it is an atomic operation, thus no synchronization needed.
         */
    ...
    public final void run() {
        messageBus.register(this); //register the microservice to the messageBus
        initialize(); //initialize microservice
    	while(!terminate){ //fetching messages loop
    	    try{
    	        Message message = messageBus.awaitMessage(this); //get a message from messages queue
    	        callbackMap.get(message.getClass()).call(message); //the the call upon the message
            }catch (InterruptedException e){
            }
        }
        messageBus.unregister(this);//at the end unregister the microservice from messageBus
    }
